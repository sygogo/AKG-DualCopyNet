...
from configs.arg_configs import post_predict_opts, init_logging
from src.utils.constants import UNK_WORD
from src.utils.string_helper import stem_word_list, stem_str_2d_list, stem_str_list
...
def main(opt):
    ...
    if opt.tune_f1_v:
        ...
        results_txt_file = open(os.path.join(opt.exp_path, "results_log_{}_{}.txt".format(result_file_suffix, opt.model_name)), "w")
        if opt.prediction_separated:
...
if __name__ == '__main__':
    ...
    post_predict_opts(parser)
    opt = parser.parse_args()

    present_absent_segmenter = '<peos>'

    logging = init_logging(log_file=opt.exp_path + '/evaluate_prediction_result.log', stdout=True)
