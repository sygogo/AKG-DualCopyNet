...
from src.inference.beam import GNMTGlobalScorer, Beam
...
class SequenceGenerator(object):
    ...
    def beam_search(self, src, src_lens, src_oov, src_mask, oov_lists, word2idx, opt, max_eos_per_output_seq=1, title=None, title_lens=None, title_mask=None):
        """
        ...
        if opt.model_name == 'dcat_seq':
            _, ner_seq, memory_bank_ner, ner_mask = self.model.Ner(src)
            memory_bank_ner = memory_bank_ner.repeat(beam_size, 1, 1)  # [batch * beam_size, max_src_len, memory_bank_size]
            ner_seq = ner_seq * self.beam_size
        ...
        decoder_state = encoder_final_state.unsqueeze(0).repeat(1, self.beam_size, 1)  # [dec_layers, batch_size * beam_size, decoder_size]
        ...
        beam_list = [
            Beam(beam_size, n_best=self.n_best, cuda=self.cuda, global_scorer=self.global_scorer, pad=self.pad_idx, eos=self.eos_idx, bos=self.bos_idx, max_eos_per_output_seq=max_eos_per_output_seq,
                 block_ngram_repeat=self.block_ngram_repeat, exclusion_tokens=exclusion_tokens) for _ in range(batch_size)]
        ...
        for t in range(1, self.max_sequence_length + 1):
            ...
            if self.copy_attn:
                decoder_input = decoder_input.masked_fill(
                    decoder_input.gt(self.model.decoder.vocab_size - 1), word2idx[UNK_WORD])
            ...
            if opt.model_name == 'dcat_seq':
                decoder_dist, decoder_state, context, attn_dist, _ = self.model.decoder(decoder_input, decoder_state, memory_bank, src_mask, max_num_oov, src_oov, ner_seq, memory_bank_ner)
            else:
                decoder_dist, decoder_state, context, attn_dist, _ = self.model.decoder(decoder_input, decoder_state, memory_bank, src_mask, max_num_oov, src_oov)
            ...
            attention_score_list.append(attn_dist)
    ...
    def sample(self, src, src_lens, src_oov, src_mask, oov_lists, max_sample_length, greedy=False, one2many=False, one2many_mode=1, num_predictions=1, perturb_std=0, entropy_regularize=False,
               title=None, title_lens=None, title_mask=None):
